...
public class TCPConnection : MonoBehaviour
{
    ...
    public string conName = "macbook";

    //ip/address of the server, 127.0.0.1 is for your own computer
    //public string conHost = "10.0.0.228";
    ...
    //read a line of text from server
    public string readSocketLine()
    {
        String result = "";
        //NetworkSocket myNetwork = GetComponentInParent<NetworkSocket>();
        if (theStream.DataAvailable)
        {
            //myNetwork.SendToServer(Time.time.ToString("F4") + "\tData available in TCPConnection.ReadSocketLine");

            result = theReader.ReadLine();
            //myNetwork.SendToServer(Time.time.ToString("F4") + "\tDone with Readline in TCPConnection.ReadSocketLine");
