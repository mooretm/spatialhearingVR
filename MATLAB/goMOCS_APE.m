...
ITI = 4; % set default intertrial interval
...
%     createFile = input('1 = YES\n2 = NO\n');
%     if createFile == 1
%         fidSubData = fopen([myPath subject '_' num2str(condition) '_MOCS.txt'],'wt');
...
fidSubData = fopen([myPath str '_MOCS.txt'],'wt');
fprintf(fidSubData,'Subject\tExperiment\tCondition\tAdaptorType\tITD\tILD\tSendTime\tResponseAz\tResponseTime\n');
...
%         plot(sig(1:round(0.5*48828/18),1),'b');
%         hold on
%         plot(sig(1:round(0.5*48828/18),2),'r');
%         hold off
...
%             hold off
%         subplot(1,2,2)
%             plot(sig(224700:224700+round(0.5*48828/18),1),'b');
%             hold on
%             plot(sig(224700:224700+round(0.5*48828/18),2),'r');
%             hold off
%         title(['ITD: ', num2str(presOrder(ii,1)), '; ILD: ', ...
%             num2str(presOrder(ii,2))]);
%     end
    ...
    %while(toc < tt + (length(whiteNoise)/48828))
    %end
    
    tt = toc;

    % present Gaussian noise
    p = set_tag_val(p,'SigL',whiteNoise(:,1));
    p = set_tag_val(p,'SigR',whiteNoise(:,2));
    p = set_tag_val(p,'SigSamps',length(whiteNoise));
    soft_trig(p,1);
    
    while toc < ((tt + (length(whiteNoise)/48828)) + 1.2)
    end
    ...
    fprintf('\n\tresponse (%.1f @ %.2fs)...\n',respazim,resptime); % report response
