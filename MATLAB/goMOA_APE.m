function VRconnection = goMOA_APE(VRconnection, subject, condition)
% function VRconnection = goMOA_APE(VRconnection, subject, condition)
%
...
%ILDs: -9*, -6, -3, 0, 3, 6, 9*
% *removed for 2nd round of data due to inability to perform condition
...
%    conds = condition;
%else
%     REMOVING ILD +/- 9 from MOA task only
%     conds = {'ITD_m300','ITD_m200','ITD_m100','ITD_0','ITD_100', ...
%         'ITD_200','ITD_300', ...
%         'ILD_m9','ILD_m6','ILD_m3','ILD_0','ILD_3','ILD_6','ILD_9'};
%     conds = {'ITD_m300','ITD_m200','ITD_m100','ITD_0','ITD_100', ...
%         'ITD_200','ITD_300', ...
%         'ILD_m6','ILD_m3','ILD_0','ILD_3','ILD_6'};
%     fprintf('\n\n\n\n\n\n\n\nALERT\n\n\n\n');
%end

%rando = datasample(conds',length(conds),1,'Replace',false);

if strcmp(condition,'itds')
    condsITD = {'ITD_m300';'ITD_m200';'ITD_m100';'ITD_0';'ITD_100'; ...
    'ITD_200';'ITD_300'};
    rando = datasample(condsITD,length(condsITD),1,'Replace',false);
    fprintf('Fixed ITDs selected\n');
    disp(rando);
elseif strcmp(condition,'ilds')
    condsILD = {'ILD_m6';'ILD_m3';'ILD_0';'ILD_3';'ILD_6'};
    rando = datasample(condsILD,length(condsILD),1,'Replace',false);
    fprintf('Fixed ILDs selected\n');
    disp(rando);
else
    fprintf('\nHey, I need a valid condition!\n')
end
...
% does a subject MAT data file exist?
if exist([myPath, subject '.mat'],'file') ~= 2
    fprintf('\n--------------------------------------\n');
    fprintf('Create new subject MAT file?\n\n')
    ...
    if createFile == 1
        ...
        data.info(11,1,1) = {'9: Unity Timestamp'};
...
% does a subject TEXT data file exist?
% if exist([myPath subject '_MOA.txt'],'file') ~= 2
%     fprintf('\n--------------------------------------\n');
%     fprintf('Create new subject TEXT file?\n\n')
%     createFile = input('1 = YES\n2 = NO\n');
%     if createFile == 1
%         fidData = fopen([myPath subject '_MOA.txt'],'wt');
%         fprintf(fidData,'Subject\tExperiment\tCondition\tBlock\tProbeITD\tProbeILD\tAdjustUP\tAdjustDOWN\tUnityTime\n');
%         fclose(fidData);
%     else
%         fprintf('Goodbye!\n');
%         return
...
%                                   .
%                                 .;;............
% begin MOA condition loop      .;;;;::::::::::::
%                                ':;;::::::::::::
%                                  ':
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ctrCond = 1:length(rando)
    condition = char(rando(ctrCond));
...
%  return
    ...
    while randoILD >= -3 && randoILD <= 3
        randoILD = randi([-6,6],1,1);
    end

    % ITD
    randoITD = 0;
    while randoITD >= -90 && randoITD <= 90
        randoITD = randi([-300,300],1,1);
    end
...
%blockNum = 20;
    ...
    fidSubData = fopen([myPath str '_MOA.txt'],'wt');
    ...
    % reset loop cancellation in Unity
    ...
    while stopLoop == 0
        ...
        plot(probe(:,2),'r');
        hold off
        title(['Adjusting: ', paradigm, '; ' num2str(condition) '; ITD: ' num2str(probeITD) '; ILD: ' num2str(probeILD)]);
    ...
    %         ylabel('Amplitude');
        ...
        fprintf('\tITD: %d\tILD: %d\tSent: %.2fs...',probeITD,probeILD,sendTime);
        fprintf('done\n');
        if strcmp(paradigm,'ILD')
    ...
    % reset loop cancellation in Unity for next condition
    ipmsg(VRconnection,'RESETEND\n');
    fprintf('\nUnity trials reset successfully\n');
