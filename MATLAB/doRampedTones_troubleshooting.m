function [sigLeft, sigRight, sigBoth] = doRampedTones_troubleshooting(cf,dur,rampdur,itd,ild,fs)
...
sigRight = db2mag(level) * sin(2*pi*cf*tlag) .* windowRightChan;
sigRight = sigRight';
...
if ild < 0
    ...
    sigRight = sigRight ./ db2mag(offset);
else
    sigLeft = sigLeft ./ db2mag(abs(ild/2));
    sigRight = sigRight .* db2mag(abs(ild/2));
